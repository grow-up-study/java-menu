...
- [ ] 요일별 카테고리 무작위 선정
  - [ ] 한 주에 같은 카테고리는 최대 2회
  - [ ] 추천할 카테고리는 `Randoms.pickNumberInRange`를 통해 생성
    - [ ] 1(일식), 2(한식), 3(중식), 4(아시안), 5(양식)
    - [ ] 추천할 수 없는 카테고리일 경우 다시 추천(일주일 2회 이상)

# 메뉴
- [ ] 추천할 메뉴는 `Randoms.shuffle()` 을 통해 순서를 섞은 후, 첫번째 값 사용

# 흐름
- 점심 메뉴 추천 시작
- 코치 이름 입력 받음(코치 클래스)
- 각 코치별 못먹는 메뉴 입력 받음(못먹는 메뉴 관리 클래스(List))
- 요일별 카테고리 추첨(RandomCategoryGenerator)
- 각 코치, 요일별 메뉴 추첨
